...
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger, } from "@rafa-resumos/ui/components/dropdown-menu";
    ...
    return (<header className="bg-neutral-50">
      <div className="mx-auto flex w-full max-w-4xl items-center justify-between gap-4 px-4 py-6 pt-8 md:px-0">
        <Link href="/" className="flex items-center gap-2 transition-opacity hover:opacity-80">
          <Image src={headerImage} alt="Rafaela" width={64} height={64} priority unoptimized className="rounded"/>
        ...
        <div className="flex items-center gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" sideOffset={8}>
              {links.map((link) => (<DropdownMenuItem key={link.href} render={<Link href={link.href}/>}>
                  {link.label}
                </DropdownMenuItem>))}
            </DropdownMenuContent>
          </DropdownMenu>
