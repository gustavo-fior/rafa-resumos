...
type PurchasePanelState = {
  ...
  status: "pending" | "paid" | "refunded" | "disputed" | "expired" | "failed";
};
...
export default function PurchasePanel({
  ...
  initialPurchase: PurchasePanelState | null;
  priceCents: number;
  slug: string;
  title: string;
}) {
  const [purchase, setPurchase] = useState<PurchasePanelState | null>(
    initialPurchase
  );
  ...
  const simulatePixMutation = useMutation({
    mutationFn: async (orderId: string) =>
      trpcClient.purchase.simulatePix.mutate({ orderId }),
    onError: (error) => {
      toast.error(error.message);
    },
    onSuccess: (data) => {
      ...
      <Card size="sm">
        <CardContent className="space-y-3">
          ...
          <Button
            ...
            className="w-full"
            disabled={isBusy}
            onClick={() => createPixMutation.mutate()}
          >
      ...
      <Card size="sm">
        <CardContent className="space-y-3">
          <div className="flex flex-col items-center gap-3">
            <img
              alt={`QR Code PIX para ${title}`}
              className="size-56 rounded-xl border bg-background"
              src={purchase.brCodeBase64}
            />
          ...
          <div className="space-y-2">
            ...
            {purchase.canSimulate && purchase.orderId ? (
              <Button
                type="button"
                variant="outline"
                className="w-full"
                disabled={simulatePixMutation.isPending}
                onClick={() => simulatePixMutation.mutate(purchase.orderId!)}
              >
