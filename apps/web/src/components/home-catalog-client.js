...
import { InputGroup, InputGroupAddon, InputGroupInput, } from "@rafa-resumos/ui/components/input-group";
...
export default function HomeCatalogClient({ categoryLabels, categoryValues, initialSections, subjectsByCategory, }) {
    ...
    const filteredSections = useMemo(() => {
        ...
        return categoriesToRender.map((category) => {
            const showSubjects = category === "medicina" && !activeSubject && !normalizedSearch;
            if (showSubjects) {
                return {
                    category,
                    kind: "subjects",
            ...
            const sourceProducts = initialSections.find((section) => section.category === category)
                ?.products ?? [];
            const products = sourceProducts.filter((product) => {
                ...
                const haystack = `${product.title} ${product.subject.name}`.toLowerCase();
    ...
    return (<>
      <section className="mt-8 space-y-6">
        <InputGroup style={{
          ...
          <InputGroupInput value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por título ou assunto"/>
        </InputGroup>

        <div className="overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <AnimatedTabs tabs={subjectTabs} value={activeSubject ?? "__all-subjects__"} onValueChange={(value) => {
      ...
      <div className="mt-10 space-y-10">
        {filteredSections.map((section) => {
            const count = section.kind === "subjects"
            ...
            return (<section key={section.category}>
              <div className="mb-3 flex items-center justify-between">
                ...
                {hasFilters ? (<Button variant="ghost" size="sm" className="text-xs text-muted-foreground font-normal" onClick={() => {
                        setActiveCategory(undefined);
                        setActiveSubject(undefined);
                        setSearch("");
                    }}>
              ...
              {section.kind === "subjects" ? (section.subjects.length ? (<div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
                    {section.subjects.map((subject) => (<SubjectCard key={subject.id} subject={subject} onClick={() => {
                  ...
                  </div>)) : section.products.length ? (<div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4">
