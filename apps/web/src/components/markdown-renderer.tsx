...
type MarkdownSegment =
  ...
  | { type: "callout"; content: string; icon: string | null; color: string | null }
...
const UNDERLINE_SPAN_PATTERN =
  /<span\s+underline\s*=\s*["']?true["']?\s*>([\s\S]*?)<\/span>/gi;
const COLOR_SPAN_PATTERN =
  /<span\s+color\s*=\s*["']([^"']+)["']\s*>([\s\S]*?)<\/span>/gi;
...
function convertToggles(content: string) {
  ...
  while (i < lines.length) {
    ...
    result.push("");
    for (const bl of body) result.push(bl);
    result.push("");
    result.push("</details>");
    result.push("");
...
function preprocessNotionMarkdown(content: string) {
  const withConversions = content
    .replace(EMPTY_BLOCK_PATTERN, "\n\n")
    .replace(UNDERLINE_SPAN_PATTERN, "<u>$1</u>")
    .replace(COLOR_SPAN_PATTERN, (_match, color: string, inner: string) => {
...
function parseColumnContents(content: string) {
  ...
  while ((match = COLUMN_PATTERN.exec(content)) !== null) {
    ...
    if (dedented) columns.push(dedented);
  }
      ...
      components={{
        a: ({ children, href, node: _node, ...props }) => (
          <a
            {...props}
            href={href}
            className="text-[#37352f] underline decoration-[#9b9a97] underline-offset-[3px] transition-colors hover:decoration-[#37352f]"
            rel="noreferrer"
            target="_blank"
          >
            {children}
        ...
        h1: ({ children, node: _node, ...props }) => (
          <h1
            {...props}
            className="mt-8 text-2xl font-semibold tracking-tight text-[#37352f]"
          >
            {children}
          </h1>
        ),
        h2: ({ children, node: _node, ...props }) => (
          <h2
            {...props}
            className="mt-6 text-xl font-semibold tracking-tight text-[#37352f]"
          >
            {children}
          </h2>
        ),
        h3: ({ children, node: _node, ...props }) => (
          <h3
            {...props}
            className="mt-5 text-lg font-semibold tracking-tight text-[#37352f]"
          >
            {children}
          </h3>
        ),
        h4: ({ children, node: _node, ...props }) => (
          <h4
            {...props}
            className="mt-4 text-base font-semibold tracking-tight text-[#37352f]"
          >
            {children}
          </h4>
        ),
        details: ({ children, node: _node, ...props }) => (
          <details
            {...props}
            className="group my-4 rounded border border-[#ededec] bg-[#fbfbfa] p-4"
          >
            {children}
          </details>
        ),
        img: ({ alt, src }) => (
          <img
            alt={alt ?? ""}
            className="my-5 w-full rounded border border-[#ededec] object-cover"
            src={src ?? ""}
          />
        ),
        summary: ({ children, node: _node, ...props }) => (
          <summary
            {...props}
            className="flex cursor-pointer list-none items-center gap-2 text-sm font-medium text-[#37352f]"
          >
            <ChevronRight
              className="size-4 shrink-0 text-[#787774] transition-transform duration-150 group-open:rotate-90"
              strokeWidth={2}
            />
        ...
        table: ({ children, node: _node, ...props }) => (
          <div className="my-5 overflow-x-auto rounded border border-[#ededec]">
  ...
  return parseMarkdownSegments(content).map((segment, index) => {
    ...
    if (segment.type === "callout") {
      const bg = segment.color
        ...
        : undefined;
      const calloutStyle: CSSProperties | undefined = bg
      ...
      return (
        <div
          key={`callout-${index}`}
          className="my-5 flex gap-3 rounded border border-[#ededec] bg-[#fbfbfa] px-4 py-3"
          style={calloutStyle}
        >
          {segment.icon ? (
            <span
              aria-hidden
              className="shrink-0 select-none pt-0.5 text-base leading-6"
            >
              {segment.icon}
            </span>
          ) : null}
    ...
    return (
      <div key={`columns-${index}`} className="reader-columns" style={style}>
        {segment.columns.map((columnContent, columnIndex) => (
          <div key={`column-${index}-${columnIndex}`} className="reader-column">
