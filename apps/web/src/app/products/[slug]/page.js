...
export default async function ProductPage({ params, }) {
    ...
    const headings = product.contentMarkdown
        ...
        : [];
    return (<main className="mx-auto w-full max-w-4xl px-4 pb-16 pt-8 md:px-0">
      <Link href="/" className="inline-flex items-center gap-1.5 text-sm text-[#787774] transition-colors hover:text-[#37352f]">
      ...
      <section className="mt-10 grid gap-8 lg:grid-cols-[1fr_22rem]">
        <div className="space-y-6">
          ...
          <h1 className="flex items-start flex-col gap-3 text-4xl">
            {product.iconEmoji ? (<span aria-hidden className="leading-none">
                {product.iconEmoji}
              </span>) : product.iconUrl ? (<img alt="" aria-hidden className="mt-1 size-8 shrink-0 object-contain" src={product.iconUrl}/>) : null}
            <span className="text-2xl font-semibold tracking-tight">
              {product.title}
            </span>
          </h1>

          {headings.length > 0 ? (<div className="border-t border-[#ededec] pt-6">
              <h2 className="text-xs font-medium uppercase tracking-wide text-[#787774]">
                Conteúdo
              </h2>
              <ul className="mt-3 space-y-1.5">
                {headings.map((heading, index) => (<li key={`${index}-${heading.text}`} className="text-sm leading-relaxed text-[#37352f]" style={{
                    paddingLeft: `${(heading.level - 1) * 1}rem`,
                }}>
                    <span className={heading.level === 1
                    ? "font-medium"
                    : heading.level === 2
                        ? ""
                        : "text-[#787774]"}>
        ...
        <aside className="space-y-4">
          {!isFree && (<Card size="sm" className="px-0.5">
              <CardContent>
          ...
          {product.hasAccess ? (<Button className="w-full" render={<Link href={`/reader/${product.slug}`}/>}>
              Abrir resumo completo
            </Button>) : session?.user ? (isFree ? (<ClaimFreeButton slug={product.slug}/>) : (<PurchasePanel initialPurchase={activePurchase} priceCents={product.priceCents} slug={product.slug} title={product.title}/>)) : (<Card>
