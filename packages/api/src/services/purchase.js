...
import { entitlements, orders, products, webhookEvents, } from "@rafa-resumos/db/schema/app";
import { env } from "@rafa-resumos/env/abacatepay";
import { and, desc, eq, gt, isNull } from "drizzle-orm";
import { checkTransparentPixCharge, createTransparentPixCharge, simulateTransparentPixCharge, } from "./abacatepay";
...
function toInternalOrderStatus(providerStatus) {
...
function truncateDescription(title) {
...
async function getOrderByIdForUser(orderId, userId) {
    return db
        ...
        .leftJoin(entitlements, and(eq(entitlements.productId, orders.productId), eq(entitlements.userId, userId), isNull(entitlements.revokedAt)))
        .where(and(eq(orders.id, orderId), eq(orders.userId, userId)))
        .limit(1)
        .then((result) => result[0]);
}
async function getOrderByProviderChargeId(providerChargeId) {
    return db
        .select(orderSelection)
        .from(orders)
        .innerJoin(products, eq(orders.productId, products.id))
        .leftJoin(entitlements, and(eq(entitlements.productId, orders.productId), eq(entitlements.userId, orders.userId), isNull(entitlements.revokedAt)))
        .where(eq(orders.providerChargeId, providerChargeId))
        .limit(1)
        .then((result) => result[0]);
}
async function grantEntitlementForOrder(order, grantedBy) {
    await db
        .insert(entitlements)
        .values({
        grantedBy,
...
async function revokeEntitlementForOrder(order) {
    await db
        .update(entitlements)
        .set({
        revokedAt: new Date(),
        ...
        .where(and(eq(entitlements.userId, order.userId), eq(entitlements.productId, order.productId), isNull(entitlements.revokedAt)));
}
async function applyProviderStateToOrder(order, providerState, source) {
...
async function refreshPendingOrder(order) {
    ...
    if (nextStatus !== order.status ||
        ...
        await applyProviderStateToOrder(order, {
...
async function findReusablePendingOrder(userId, productId) {
    const existing = await db
        .select(orderSelection)
        .from(orders)
        .innerJoin(products, eq(orders.productId, products.id))
        .leftJoin(entitlements, and(eq(entitlements.productId, orders.productId), eq(entitlements.userId, orders.userId), isNull(entitlements.revokedAt)))
        .where(and(eq(orders.userId, userId), eq(orders.productId, productId), eq(orders.status, "pending"), gt(orders.paymentExpiresAt, new Date())))
...
export async function claimFreeProduct(input) {
    ...
    if (product.hasAccess) {
        return { alreadyOwned: true, productId: product.id };
    }
    await db
        .insert(entitlements)
        .values({
    ...
    return { alreadyOwned: false, productId: product.id };
}
export async function getActivePurchaseForProductSlug(userId, slug) {
    const existing = await db
        .select(orderSelection)
        .from(orders)
        .innerJoin(products, eq(orders.productId, products.id))
        .leftJoin(entitlements, and(eq(entitlements.productId, orders.productId), eq(entitlements.userId, orders.userId), isNull(entitlements.revokedAt)))
        .where(and(eq(orders.userId, userId), eq(products.slug, slug), eq(orders.status, "pending")))
        .orderBy(desc(orders.createdAt))
        .limit(1)
        .then((result) => result[0]);
    if (!existing ||
...
export async function createPixPurchase(input) {
    ...
    if (product.priceCents <= 0) {
        throw new Error("This product does not have a valid PIX amount configured yet.");
    }
    const reusableOrder = await findReusablePendingOrder(input.userId, product.id);
    if (reusableOrder) {
...
export async function simulatePixPurchase(input) {
    ...
    if (!isDevModeApiKey()) {
        throw new Error("PIX simulation is only available with a development API key.");
    }
...
export async function processAbacatepayWebhook(payload, rawPayload) {
    const existingEvent = await db
        ...
        .from(webhookEvents)
        .where(and(eq(webhookEvents.provider, "abacatepay"), eq(webhookEvents.providerEventId, payload.id)))
        .limit(1)
        .then((result) => result[0]);
