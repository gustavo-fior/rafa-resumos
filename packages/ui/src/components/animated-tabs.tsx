...
export function AnimatedTabs({
  ...
  return (
    <div className="relative flex w-fit flex-col items-center rounded-full">
      ...
      >
        <div className="relative flex w-full justify-center bg-primary/8">
          {normalizedTabs.map((tab) =>
            tab.href ? (
              <a
                ...
                className={cn(
                  "flex h-8 items-center rounded-full p-3 px-4 font-medium text-neutral-800 text-sm/5.5 cursor-pointer"
                )}
                tabIndex={-1}
              >
                {tab.label}
        ...
        {normalizedTabs.map((tab) => {
          ...
          return tab.href ? (
            <a
              ...
              ref={activeRef}
              onClick={() => handleTabClick(tab.value)}
              className="flex h-8 items-center rounded-full p-3 px-4 font-medium text-sm/5.5 cursor-pointer"
            >
              {tab.label}
            </a>
          ) : (
